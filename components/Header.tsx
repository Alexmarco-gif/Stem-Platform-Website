    ...
    <header className={`site-header header-surface ${open ? "menu-open" : ""}`}>
      <div className="site-shell header-inner">
        ...
        <nav className="desktop-nav" aria-label="Primary navigation">
          {links.map((link) => <Link key={link.href} href={link.href} className={pathname === link.href ? "active" : ""} aria-current={pathname === link.href ? "page" : undefined}>{link.label}</Link>)}
        </nav>
      ...
      <div id="mobile-navigation" className={`mobile-nav ${open ? "open" : ""}`}>
        <nav className="site-shell" aria-label="Mobile navigation">
          ...
          {links.map((link, index) => <Link key={link.href} href={link.href}><span>0{index + 1}</span>{link.label}<b aria-hidden="true">→</b></Link>)}
